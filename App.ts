let dieContainer: JQuery<HTMLElement> = $(`#dieContainer`); // this is the div that holds an individual die

let pipsActive: boolean = false; // this controls the state of displaying pips or not

let allDieObject: HTMLDivElement[] = <HTMLDivElement[]>[...document.getElementsByClassName("die")]; // this is object that contains each die-div on screen - use: allDieObject[i].innerText
// let allDieObjectArray = []

let currentDice: Die[] = []; // this array will hold all the die objects
...
class Die {
  ...
  roll() {
    ...
    let newValue: number = Math.floor(Math.random() * 6 + 1);
    this.value = newValue;
  }
}
...
$(`#newdie`).click(function () {
  ...
  let myDie: Die = new Die(); // myDie is a new instance of the Die class
  myDie.roll(); //  sets the initial value of the die from 0 to a random die value

  //sets styles and text value
  let mydieholder: JQuery<HTMLElement> = $(`<div class="dieParent col-1 my-3">`); // this div holds the die
  let myPageDie: JQuery<HTMLElement> = $(`<div class="die py-3">`) // this div is the representation of the die
  ...
  myPageDie.click(() => {
    ...
    let thisDieId: number = myDie.id; // gets the id of the die, so it knows which is being clicked

    let indexToRoll: number = currentDice.findIndex(idProp);

    function idProp(objectHere: Die): boolean {
  ...
  myPageDie.dblclick(() => {
    let thisDieId2: number = myDie.id; // gets the id of the die, so it knows which one is being double clicked
    console.log(`double click has fired`);
    // console.log(currentDice);

    let indexToKill: number = currentDice.findIndex(idProp); // gets the index of the first element that passes the test described in the function below
...
$(`#reroll`).click(function () {
  for (let i = 0; i < currentDice.length; i++) {
    let currentDie: Die = currentDice[i];

    currentDie.roll(); // runs through the array, calling the roll function on each object

    let newRoll: number = currentDie.value; // stores the value in a variable
  ...
  var audio: HTMLAudioElement = new Audio("RollSound.mp3");
  audio.play();
});
...
function setDisplay() {
  ...
  if (pipsActive) {
    ...
    for (let i = 0; i < allDieObject.length; i++) {
      ...
      let textCompare: string = allDieObject[i].innerText; // this stores their text - which matches the value
